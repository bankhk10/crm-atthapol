  ...
  return (
    <Stack spacing={1.25}>
      ...
      <Box sx={{ position: "relative", display: "flex", alignItems: "center" }}>
        {canScrollLeft && (
          <IconButton onClick={() => scroll("left")} size="small" sx={{ position: "absolute", left: 2, zIndex: 1, bgcolor: "rgba(255,255,255,0.7)", "&:hover": { bgcolor: "white" } }}>
        ...
        {canScrollRight && (
          <IconButton onClick={() => scroll("right")} size="small" sx={{ position: "absolute", right: 2, zIndex: 1, bgcolor: "rgba(255,255,255,0.7)", "&:hover": { bgcolor: "white" } }}>
            <ArrowForwardIosIcon fontSize="inherit" />
          </IconButton>
        )}
      </Box>

      {/* Lightbox (ไม่มีการเปลี่ยนแปลง) */}
      <Dialog
        open={open}
      ...
      >
        <Box sx={{ position: "relative", p: 0, width: { xs: 320, sm: 560, md: 820 }, height: { xs: 320, sm: 560, md: 560 } }}>
          <IconButton onClick={() => setOpen(false)} sx={{ position: "absolute", top: 6, right: 6, zIndex: 2, color: "white", bgcolor: "error.main", "&:hover": { bgcolor: "error.dark" } }}>
            <CloseIcon />
          </IconButton>
          {imgs.length > 1 && (
            <>
