...
type Order = "asc" | "desc";
type SortableKeys = "nameTH" | "brand" | "status" | "category" | "createdAt";

interface HeadCell { id: SortableKeys; label: string; numeric?: boolean; disablePadding?: boolean; }

const headCells: readonly HeadCell[] = [
  { id: "nameTH", label: "Name" },
  { id: "brand", label: "Description" },
...
function getComparator<Key extends keyof any>(order: Order, orderBy: Key) {
  return order === "desc"
    ? (a: { [key in Key]: number | string | null }, b: { [key in Key]: number | string | null }) =>
        descendingComparator(a, b, orderBy)
    : (a: { [key in Key]: number | string | null }, b: { [key in Key]: number | string | null }) =>
        -descendingComparator(a, b, orderBy);
}

function EnhancedTableHead({ order, orderBy, onRequestSort }: { order: Order; orderBy: SortableKeys; onRequestSort: (e: React.MouseEvent<unknown>, p: SortableKeys) => void; }) {
  ...
  return (
    <TableHead sx={{ bgcolor: 'grey.100', '& .MuiTableCell-root': { bgcolor: 'grey.100' } }}>
      <TableRow>
        ...
        <TableCell align="center" padding="normal">View</TableCell>
        <TableCell align="center" padding="normal">Edit</TableCell>
        <TableCell align="center" padding="normal">Delete</TableCell>
      </TableRow>
    </TableHead>
  );
}
...
export function ProductsTable({ products }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter((p) => [p.productCode, p.nameTH, p.category, p.brand ?? "", p.status].join(" ").toLowerCase().includes(q));
  }, [products, query]);
  ...
  const visibleRows = useMemo(
    () => [...filtered].sort(getComparator(order, orderBy)).slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage),
  ...
  return (
    <Paper variant="outlined" sx={{ p: 0, borderRadius: 2, overflow: 'hidden' }}>
      <Toolbar sx={{ pl: { sm: 2 }, pr: { xs: 1, sm: 1 } }}>
        <Typography sx={{ flex: '1 1 100%' }} variant="h6" id="tableTitle" component="div">รายการสินค้า</Typography>
        <Box sx={{ position: 'relative', width: { xs: 200, sm: 260, md: 360 } }}>
          <TextField fullWidth value={query} onChange={(e) => setQuery(e.target.value)} placeholder="ค้นหา (รหัส/ชื่อ/หมวด/ยี่ห้อ/สถานะ)" InputProps={{ startAdornment: (<SearchIcon fontSize="small" style={{ marginRight: 8 }} />) }} size="small" />
        </Box>
      </Toolbar>
          ...
          <TableBody>
            {visibleRows.map((p) => (
              <TableRow hover key={p.id}>
                ...
                <TableCell sx={{ whiteSpace: 'nowrap' }}>{p.brand ?? '-'}</TableCell>
                <TableCell>
                ...
                <TableCell align="center" sx={{ width: 80 }}>
                  <Tooltip title="ดูรายละเอียด" placement="top">
                    <IconButton component={Link} href={`/dashboard/products/${p.id}`} aria-label="view" size="small">
