...
export async function getEmployeeActivities(employeeId: string): Promise<EmployeeActivityItem[]> {
  ...
  for (const it of interactions) {
    ...
    let code = "-";
    const where: any = {};
    if ((it as any).customer) {
      const cust = (it as any).customer;
      if (cust.customerType === "DEALER") title = "เข้าพบร้านค้า";
      else if (cust.customerType === "SUB_DEALER") title = "เข้าพบซับดีลเลอร์";
