    ...
    if (val.type === "FARMER" || val.type === "BROKER") {
      if (!phone && !cphone) {
        ctx.addIssue({
          code: "custom",
          path: ["contactPhone"],
          message: "กรุณากรอกเบอร์โทร (บุคคล)",
        });
      }
    }
  });
      ...
      if (values.type === "DEALER") {
        ...
        await (tx as any).subDealerDetail.create({
          data: {
            ...
            averageMonthlyPurchase:
              values.averageMonthlyPurchase !== undefined &&
              values.averageMonthlyPurchase !== null &&
              String(values.averageMonthlyPurchase) !== ""
        ...
        const fd = await (tx as any).farmerDetail.create({
          data: {
            ...
            areaSize:
              areaSum ||
              (values.farmSize !== undefined &&
              values.farmSize !== null &&
              String(values.farmSize) !== ""
        ...
        if (plots.length) {
          await (tx as any).farmPlot.createMany({
            data: plots.map((p: any) => ({
              ...
              latitude:
                p.latitude !== undefined && p.latitude !== null && String(p.latitude) !== ""
                  ? Number(p.latitude)
                  : undefined,
              longitude:
                p.longitude !== undefined && p.longitude !== null && String(p.longitude) !== ""
                  ? Number(p.longitude)
                  : undefined,
              plantingArea:
  ...
  await runWithRequestContext({ userId: session?.user?.id }, async () => {
    await prisma.$transaction(async (tx) => {
      ...
      if (values.type === "DEALER") {
        ...
        const existing: any[] = await (tx as any).farmPlot.findMany({
          where: { farmerDetailId: fd.id },
          select: { id: true },
        });
        const existingIds = new Set(
          (existing.map((r: any) => r.id).filter(Boolean) as Array<string | number>).map(String),
        );
        const submittedIds = new Set(
          (plots.map((p: any) => p.id).filter(Boolean) as Array<string | number>).map(String),
        );
