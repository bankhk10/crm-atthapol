...
export default async function CustomerDetailPage({
  ...
  const age = customer.birthDate
    ? Math.floor(
        (Date.now() - new Date(customer.birthDate).getTime()) / (1000 * 60 * 60 * 24 * 365.25),
      )
    : null;
  const addressLine = [
    customer.address,
            ...
            {customer.type === "SUBDEALER" && (
              <Section title="ข้อมูล SubDealer">
                <Info
                  ...
                  value={(customer as any).parentDealer ?? "-"}
                />
            ...
            {customer.type === "BROKER" && (
              <Section title="ข้อมูล Broker">
                ...
                <Info
                  ...
                  value={(customer as any).currentCropVolume ?? "-"}
                />
                <Info
                  label="จำนวนเกษตรกรในเครือ"
                  value={(customer as any).farmerNetworkCount ?? "-"}
                />
                ...
                <Info
                  ...
                  value={(customer as any).creditTermForFarmers ?? "-"}
                />
                <Info
                  label="มูลค่าสารเคมี/รอบ (บาท)"
                  value={(customer as any).agriChemValuePerCycle ?? "-"}
                />
                <Info
                  label="ปริมาณสารเคมี/รอบ"
                  value={(customer as any).agriChemQtyPerCycle ?? "-"}
                />
