  ...
  return (
    <>
      ...
      <Box display="grid" gridTemplateColumns={{ xs: "1fr", md: "repeat(4, 1fr)" }} gap={2} mt={1}>
        <Box>
          <KpiCard
            label={data.kpis.revenue.label}
            value={`${currency(totalRevenue)} บาท`}
            helpText={data.kpis.revenue.help}
            delta={data.kpis.revenue.delta}
          />
        </Box>
        <Box>
          <KpiCard
            label={data.kpis.orders.label}
            value={data.kpis.orders.value}
            helpText={data.kpis.orders.help}
            delta={data.kpis.orders.delta}
          />
        </Box>
        <Box>
          <KpiCard
            label={data.kpis.aov.label}
            value={`${currency(data.kpis.aov.value)} บาท`}
            helpText={data.kpis.aov.help}
            delta={data.kpis.aov.delta}
          />
        </Box>
        <Box>
          <KpiCard
            label={data.kpis.newCustomers.label}
            value={data.kpis.newCustomers.value}
            helpText={data.kpis.newCustomers.help}
            delta={data.kpis.newCustomers.delta}
          />
        </Box>
      </Box>
            ...
            <Stack spacing={1.5}>
              ...
              <Stack direction="row" spacing={2}>
                <Typography variant="body2" color="text.secondary">
                  ยอดขายเฉลี่ย/เดือน:{" "}
                  {currency(Math.round(data.trend.reduce((a, b) => a + b, 0) / data.trend.length))}{" "}
                  บาท
                </Typography>
              </Stack>
            </Stack>
        ...
        <Box>
          <Paper variant="outlined" sx={{ p: 2.5, height: "100%" }}>
            <Stack spacing={1.25}>
              ...
              <BarList
                items={data.topProducts.map((p, idx) => ({
                  label: p.name,
                  value: p.revenue,
