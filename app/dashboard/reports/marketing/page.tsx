  ...
  return (
    <>
      ...
      <Box display="grid" gridTemplateColumns={{ xs: "1fr", md: "repeat(4, 1fr)" }} gap={2} mt={1}>
        ...
        <Box>
          <KpiCard
            label={data.kpis.spend.label}
            value={`${currency(data.kpis.spend.value)} บาท`}
            delta={data.kpis.spend.delta}
          />
        </Box>
        <Box>
          <KpiCard
            label={data.kpis.cpl.label}
            value={`${Number(data.kpis.cpl.value).toFixed(1)} บาท`}
            delta={data.kpis.cpl.delta}
          />
        </Box>
        <Box>
          <KpiCard
            label={data.kpis.ctr.label}
            value={`${Number(data.kpis.ctr.value).toFixed(1)}%`}
                  ...
                  {data.channels.map((ch) => (
                    <TableRow key={ch.label} hover>
                      ...
                      <TableCell align="right">
                        {((ch.clicks / Math.max(1, ch.impressions)) * 100).toFixed(2)}%
                      </TableCell>
          ...
          <Paper variant="outlined" sx={{ p: 2.5, height: "100%" }}>
            ...
            <BarList
              items={data.funnel.map((f, i) => ({
                ...
                value: f.value,
                color: ["#90caf9", "#64b5f6", "#42a5f5", "#1e88e5", "#1565c0", "#0d47a1"][i % 6],
              }))}
              total={funnelTotal}
            />
          </Paper>
              ...
              <TableBody>
                {data.campaigns.map((c) => (
                  <TableRow key={c.name} hover>
                    ...
                    <TableCell align="right">
                      {(c.spend / Math.max(1, c.leads)).toFixed(1)} บาท
                    </TableCell>
