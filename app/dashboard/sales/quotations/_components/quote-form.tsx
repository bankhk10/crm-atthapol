...
function computeTotals(items: QuoteItemInput[], vatRate: number, shippingFee: number, otherCharges: number) {
  let subTotal = 0;
  let discountTotal = 0;
  let taxAmount = 0;
...
export function QuoteForm({
  ...
  const [quoteDate, setQuoteDate] = useState<string | null>(initial?.quoteDate ?? new Date().toISOString().slice(0, 10));
  ...
  const [items, setItems] = useState<QuoteItemInput[]>(initial?.items?.length ? initial.items : [{ ...DEFAULT_ITEM }]);
  ...
  return (
    <Stack spacing={2} sx={{ maxWidth: 1200, mx: "auto", p: { xs: 1.5, md: 2 }, bgcolor: "#fff" }}>
      ...
      <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
        <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={th}>
          <DatePicker
            ...
            onChange={(newValue) => setQuoteDate(newValue ? newValue.toISOString().slice(0, 10) : null)}
        ...
        <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={th}>
          <DatePicker
            ...
            onChange={(newValue) => setValidUntil(newValue ? newValue.toISOString().slice(0, 10) : null)}
            slotProps={{ textField: { fullWidth: true } }}
          />
        </LocalizationProvider>
      </Stack>
      ...
      <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
        <TextField select label="เงื่อนไขการชำระ" value={paymentCondition} onChange={(e) => setPaymentCondition(e.target.value as any)} fullWidth>
          <MenuItem value="PREPAID">ชำระก่อนส่ง</MenuItem>
          <MenuItem value="POSTPAID">เครดิตเทอม</MenuItem>
        </TextField>
        <TextField label="เครดิตเทอม (วัน)" type="number" value={creditTermDays} onChange={(e) => setCreditTermDays(e.target.value === "" ? "" : Number(e.target.value))} fullWidth disabled={paymentCondition !== "POSTPAID"} />
        <TextField label="วิธีจัดส่ง" value={shippingMethod} onChange={(e) => setShippingMethod(e.target.value)} fullWidth />
      </Stack>
      <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
        <TextField select label="สกุลเงิน" value={currency} onChange={(e) => setCurrency(e.target.value)} fullWidth>
          <MenuItem value="THB">THB</MenuItem>
        </TextField>
        <FormControlLabel control={<Checkbox checked={vatIncluded} onChange={(e) => setVatIncluded(e.target.checked)} />} label="ราคารวมภาษี" />
        <TextField label="VAT (%)" type="number" value={vatRate} onChange={(e) => setVatRate(Number(e.target.value || 0))} sx={{ width: { xs: "100%", md: 140 } }} />
      </Stack>
        ...
        {items.map((it, idx) => (
          <Paper key={idx} variant="outlined" sx={{ p: 1.5 }}>
            <Stack direction={{ xs: "column", md: "row" }} spacing={{ xs: 1.5, md: 2 }} alignItems={{ xs: "stretch", md: "center" }} justifyContent="space-between" flexWrap="wrap">
              ...
              <TextField label="จำนวน" type="number" value={it.qty} onChange={(e) => { const val = e.target.value.replace(/^0+(?=\d)/, ""); const next = [...items]; next[idx] = { ...next[idx], qty: val === "" ? 0 : Math.max(0, Number(val)) }; setItems(next); }} sx={{ width: { xs: "100%", md: 120 } }} required />
              <TextField label="ส่วนลด (บาท)" type="number" value={it.discountAmount} onChange={(e) => { const val = e.target.value.replace(/^0+(?=\d)/, ""); const next = [...items]; next[idx] = { ...next[idx], discountAmount: val === "" ? "" : Math.max(0, Number(val)) }; setItems(next); }} sx={{ width: { xs: "100%", md: 160 } }} />
              <Button color="error" onClick={() => setItems((prev) => prev.filter((_, i) => i !== idx))}>ลบ</Button>
            </Stack>
          </Paper>
        ))}
      ...
      <Box sx={{ backgroundColor: "#d9d9dbff", borderRadius: 2, px: 2, py: 2 }}>
        <Typography variant="h6" fontWeight={960}>ค่าใช้จ่ายและหมายเหตุ</Typography>
      </Box>
      <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
        ...
        <TextField label="ส่วนลดทั้งบิล (บาท)" type="number" value={orderDiscount} onChange={(e) => setOrderDiscount(e.target.value === "" ? "" : Math.max(0, Number(e.target.value)))} fullWidth />
      </Stack>
      <TextField label="หมายเหตุ" value={note} onChange={(e) => setNote(e.target.value)} fullWidth />

      {/* สรุปยอด */}
      <Box sx={{ backgroundColor: "#d9d9dbff", borderRadius: 2, px: 2, py: 2 }}>
        <Typography variant="h6" fontWeight={960}>สรุปยอด</Typography>
      </Box>
      <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
      ...
      <Stack direction={{ xs: "column", sm: "row" }} spacing={2} justifyContent="center" alignItems="center">
        <Button component={Link} href="/dashboard/sales/quotes" variant="contained" startIcon={<CloseIcon />} sx={{ bgcolor: red[600], color: "white", "&:hover": { bgcolor: red[700] }, fontWeight: 800, px: 3 }}>ยกเลิก</Button>
        <Button onClick={handleSubmit} variant="contained" disabled={!canSubmit || isSubmitting} startIcon={<CheckIcon />} sx={{ bgcolor: blue[600], color: "white", "&:hover": { bgcolor: blue[700] }, fontWeight: 800, px: 3 }}>{submitLabel}</Button>
