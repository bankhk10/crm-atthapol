    ...
    if (releaseQty > 0) {
      ...
      await (tx as any).stockMovement.create({
        data: {
          stockId: r.stockId,
          productId: (
            await tx.stock.findUnique({ where: { id: r.stockId }, select: { productId: true } })
          ).productId,
...
export async function PUT(req: NextRequest, context: { params: Promise<{ orderId: string }> }) {
  ...
  try {
    ...
    try {
      const existsHeader = await (prisma as any).saleOrder.findUnique({
        where: { id: orderId },
        select: { paymentCondition: true },
      });
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      if (changingStatus) {
        ...
        const isFreeChange =
          (prevStatus === "DRAFT" && nextStatus === "CONFIRMED") ||
          (prevStatus === "CONFIRMED" && nextStatus === "DRAFT");
        if (!isFreeChange) {
        ...
        data: {
          ...
          approvedByUserId:
            nextStatus === "APPROVED"
              ...
              : prevStatus === "APPROVED" && nextStatus !== "APPROVED"
                ? null
                : (exists as any).approvedByUserId,
    ...
    if (err instanceof Error && err.message === "LOCKED_APPROVED") {
      return NextResponse.json(
        { error: "เอกสารถูกอนุมัติแล้ว ต้องเปลี่ยนสถานะเป็นรออนุมัติจึงจะแก้ไขได้" },
        { status: 400 },
      );
    }
    if (err instanceof Error && err.message === "NO_APPROVE") {
      return NextResponse.json(
        { error: "ไม่มีสิทธิ์เปลี่ยนแปลงสถานะ/อนุมัติเอกสาร" },
        { status: 403 },
      );
    }
    ...
    if (err instanceof Error && err.message === "PROMO_NOT_SUPPORTED") {
      return NextResponse.json(
        { error: "ลูกค้ารายนี้ไม่รองรับวงเงินส่งเสริมการขาย" },
        { status: 400 },
      );
    }
    ...
    if (err instanceof Error && err.message === "PROMO_REFUND_TARGET_MISSING") {
      return NextResponse.json(
        { error: "ไม่สามารถคืนวงเงินส่งเสริมการขายเดิมได้" },
        { status: 400 },
      );
    }
